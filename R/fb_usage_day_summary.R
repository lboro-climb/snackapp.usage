#' fb_usage_day_summary
#' 
#' @description A function to produce a daily summary of Fitbit usage data
#'
#' @param data A dataframe 
#'
#' @return A dataframe summarising participants daily Fitbit usage 
#' @export
#'
#' @examples
fb_usage_day_summary <- function(data) {
  ...
  data <- data %>%
    dplyr::mutate(
      ...
      year = lubridate::year(event_start),
      month = lubridate::month(event_start),
      day = lubridate::day(event_start)
    ) %>%
