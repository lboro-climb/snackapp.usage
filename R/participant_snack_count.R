#' Title participant_snack_count
#' 
#' @description A function to count the number of screens viewed by a participant within the SnackApp
#'
#' @param data A dataframe of SnackApp usage data
#'
#' @return A dataframe of summary information regarding SnackApp usage
#' @export
#'
#' @examples
participant_snack_count <- function(data){
  df <- data %>%
    dplyr::group_by(id) %>%
  ...
  total <- data %>% dplyr::group_by(id) %>% dplyr::filter(Metric == "background") %>% dplyr::count()
  ...
  results <- dplyr::full_join(results, total, by = c("id" = "id")) %>%
    dplyr::mutate(
      id = as.double(id)
    )
  colnames(results) <- (c("id", "bouts_dashboard", "bouts_my_stat", "bouts_my_goal", "bouts_forum",
                          "bouts_case_studies", "bouts_faq", "bouts_profile", "bouts_onboarding", "bouts_planner", "total_bouts"))
  return(results)
}
